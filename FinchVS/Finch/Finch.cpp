...
namespace {
    ...
    public:
        MutexLocker(HANDLE mutex)
            ...
            locked = WaitForSingleObject(mutex, INFINITE) == WAIT_OBJECT_0;
        }
        MutexLocker(HANDLE mutex, bool tryOnly)
            : mtx(mutex), locked(false) {
            if (tryOnly) {
				locked = WaitForSingleObject(mutex, 0) == 0;
            }
            else {
				locked = WaitForSingleObject(mutex, INFINITE) == 0;
            }
        }
        ~MutexLocker() {
            unlock();
...
/**
 * Windows specific keep alive entry point
 */
DWORD WINAPI windowsKeepAliveEntryPoint(LPVOID lpParam) {
	Finch * pthX = static_cast<Finch*>(lpParam);   // cast from void to Finch object
...
Finch::Finch() : initialized(false), pimpl(new Impl) {
	...
	pimpl->threadHandle = CreateThread(
		NULL,                   // default security attributes
		0,                      // use default stack size  
		windowsKeepAliveEntryPoint,       // thread function name
		this,          // argument to thread function 
		0,                      // use default creation flags 
		&pimpl->threadid);   // returns the thread identifier 
...
int Finch::disConnect() {
    ...
    if (pimpl->finch_handle) {
		...
		WaitForSingleObject(pimpl->threadHandle, INFINITE);
